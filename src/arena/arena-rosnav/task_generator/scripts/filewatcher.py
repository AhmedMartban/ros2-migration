...
def run(namespace: Optional[str] = None):
    ...
    if namespace is None:
        namespace = rosros.get_namespace()
    ...
    FILE_TASK_CONFIG = os.path.join(
        rosros.RosPack().get_path("arena_bringup"),
        "configs",
        "task_generator.yaml"
    )
    ...
    class TaskConfigHandler(watchdog.events.FileSystemEventHandler):
        ...
        def reconfigure():
            ...
            rosros.logdebug("SENSING CHANGE OF TASK_MODE PARAMS")
...
if __name__ == "__main__":
    rosros.init_node("task_generator_filewatcher")
    run()
