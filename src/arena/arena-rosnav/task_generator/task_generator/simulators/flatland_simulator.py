...
class FlatlandSimulator(BaseSimulator):
    ...
    _move_model_srv: rosros.ServiceProxy
    _spawn_model_srv: rosros.ServiceProxy
    _spawn_model_from_string_srv: rosros.ServiceProxy
    _spawn_models_from_string_srv: rosros.ServiceProxy
    _delete_model_srv: rosros.ServiceProxy
    _delete_models_srv: rosros.ServiceProxy

    _resume_srv: rosros.ServiceProxy
    _pause_srv: rosros.ServiceProxy
    ...
    def __init__(self, namespace):
        ...
        self._move_robot_pub = rosros.Publisher(
        ...
        rosros.wait_for_service(self._namespace("move_model"), timeout=Config.General.WAIT_FOR_SERVICE_TIMEOUT)
        rosros.wait_for_service(self._namespace("spawn_model"), timeout=Config.General.WAIT_FOR_SERVICE_TIMEOUT)
        rosros.wait_for_service(self._namespace("delete_model"), timeout=Config.General.WAIT_FOR_SERVICE_TIMEOUT)

        self._move_model_srv = rosros.ServiceProxy(
            self._namespace("move_model"), flatland_srvs.MoveModel, persistent=True
        )
        self._spawn_model_srv = rosros.ServiceProxy(
            self._namespace("spawn_model_from_string"), flatland_srvs.SpawnModel
        )
        self._spawn_model[ModelType.YAML] = rosros.ServiceProxy(
            self._namespace("spawn_model_from_string"), flatland_srvs.SpawnModel
        )
        self._spawn_models_from_string_srv = rosros.ServiceProxy(
            self._namespace("spawn_models_from_string"), flatland_srvs.SpawnModels
        )
        self._delete_model_srv = rosros.ServiceProxy(
            self._namespace("delete_model"), flatland_srvs.DeleteModel
        )
        self._delete_models_srv = rosros.ServiceProxy(
            self._namespace("delete_models"), flatland_srvs.DeleteModels
        )

        self._pause_srv = rosros.ServiceProxy(
            self._namespace("pause"), std_srvs.Empty
        )

        self._resume_srv = rosros.ServiceProxy(
    ...
    def _pause(self):
        ...
        for _ in range(self.SERVICE_CALL_TRIES):
            try: self._pause_srv()
            except rosros.ServiceException: time.sleep(0.1)
            else: return
        else: rosros.logerr(f"failed to pause in {self.SERVICE_CALL_TRIES} tries")
    ...
    def _resume(self):
        ...
        else: rosros.logerr(f"failed to resume in {self.SERVICE_CALL_TRIES} tries")
